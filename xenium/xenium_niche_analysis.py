...
def visualize_spatial_distribution(
    ...
    if show_test:
        for sample in (
            adata[adata.obs["train_test_split"] == "test"].obs["sample"].unique()
        ):
    ...
    plt.tight_layout()
    plt.savefig(
        f"figures/xenium_sample1_proseg_spatial_hub_subset_analysis_{labels_key}.png"
    )
    scatter.collections[0].set_visible(False)
    plt.savefig(
        f"figures/xenium_sample1_proseg_spatial_hub_subset_analysis_{labels_key}.svg"
    )
...
optimal_k_hub, _ = find_optimal_clusters(
    high_interacting_counts_norm_no_cluster, random_state=seed
)
...
high_interacting_counts_norm = attention_patterns.compute_communication_hubs(
    attention_quantile_threshold=quantile, n_clusters=optimal_k_hub, random_state=seed
)
adata.obs["hub_cluster"] = high_interacting_counts_norm.loc[
...
visualize_spatial_distribution(
    adata,
    labels_key="hub_cluster",
    palette=cluster_palette,
)

# %% Get hub cluster labels and create composition plot
# Calculate the mean composition for each hub cluster
sender_cell_type_composition = high_interacting_counts_norm.groupby(
    "hub_cluster"
).mean()
sender_cell_type_composition_norm = sender_cell_type_composition.div(
    sender_cell_type_composition.sum(axis=1), axis=0
)
...
sender_cell_type_composition_norm.plot(
    ...
    stacked=True,
    color=[
        CELL_TYPE_PALETTE.get(cell_type, "#888888")
        for cell_type in sender_cell_type_composition_norm.columns
    ],
    figsize=(12, 8),
)
...
high_interacting_counts_norm["receiver_label"] = adata.obs[labels_key].loc[
    high_interacting_counts_norm.index
]

# Calculate the mean composition for each hub cluster
receiver_cell_type_composition = pd.DataFrame(
    high_interacting_counts_norm[["receiver_label", "hub_cluster"]]
    .groupby("hub_cluster")
    .value_counts()
).reset_index()
receiver_cell_type_composition = receiver_cell_type_composition.pivot(
    columns="receiver_label", index="hub_cluster", values="count"
)
receiver_cell_type_composition_norm = receiver_cell_type_composition.div(
    receiver_cell_type_composition.sum(axis=1), axis=0
)
...
receiver_cell_type_composition_norm.plot(
    ...
    color=[
        CELL_TYPE_PALETTE.get(cell_type, "#888888")
        for cell_type in receiver_cell_type_composition_norm.columns
    ],
    figsize=(12, 8),
)
...
plt.tight_layout()
plt.savefig(
    "figures/xenium_hub_cluster_composition_receiver.png", dpi=300, bbox_inches="tight"
)
plt.show()
...
nn_labels_melted = nn_labels.reset_index().melt(
    id_vars="index", var_name="neighbor_position", value_name="cell_type"
)
...
adata.obs["composition_cluster"] = neighbor_composition_norm.loc[
    adata.obs_names, "composition_cluster"
]

# %% Plot the cells on spatial plot colored by the composition clusters
visualize_spatial_distribution(
    adata, labels_key="composition_cluster", palette=cluster_palette
)
...
cell_type_composition.plot(
    ...
    color=[
        CELL_TYPE_PALETTE.get(cell_type, "#888888")
        for cell_type in cell_type_composition.columns
    ],
    figsize=(12, 8),
)
...
plt.tight_layout()
plt.savefig(
    "figures/xenium_composition_cluster_composition.png", dpi=300, bbox_inches="tight"
)
plt.savefig(
    "figures/xenium_composition_cluster_composition.svg", dpi=300, bbox_inches="tight"
)
plt.show()
...
print(
    f"Adjusted Rand Index between hub clusters and composition clusters: {ari_comp_score:.4f}"
)
...
print(
    f"Adjusted Mutual Information between hub clusters and composition clusters: {ami_comp_score:.4f}"
)
...
print(
    f"Adjusted Rand Index between composition clusters and cell type labels: {ari_cell_type_score:.4f}"
)
...
print(
    f"Adjusted Mutual Information between hub clusters and cell type labels: {ami_cell_type_score:.4f}"
)
...
bars = sns.barplot(
    data=scores_df, x="Metric", y="Score", hue="Comparison", palette="Set2"
)
plt.ylim(0, 1)
...
plt.savefig(
    "figures/xenium_clustering_comparison_metrics.png", dpi=300, bbox_inches="tight"
)
plt.savefig(
    "figures/xenium_clustering_comparison_metrics.svg", dpi=300, bbox_inches="tight"
)
plt.show()

# %% Zooming into two specific hub communication clusters
hub_cluster = 9
high_interacting_counts_cluster = high_interacting_counts_norm[
    high_interacting_counts_norm["hub_cluster"] == hub_cluster
]
...
high_interacting_ct_props = (
    high_interacting_counts_cluster_melted[
        ["receiver_label", "neighbor_label", "proportion"]
    ]
    .groupby(["receiver_label", "neighbor_label"])
    .mean()
)
high_interacting_ct_props = high_interacting_ct_props.reset_index()
high_interacting_ct_props = high_interacting_ct_props[
    high_interacting_ct_props["receiver_label"]
    != high_interacting_ct_props["neighbor_label"]
]

# Weight by receiver cell proportions in the cluster
receiver_weights = receiver_cell_type_composition_norm.loc[hub_cluster]
high_interacting_ct_props["receiver_weight"] = (
    high_interacting_ct_props["receiver_label"].map(receiver_weights).astype(float)
)
high_interacting_ct_props["weighted_proportion"] = (
    high_interacting_ct_props["proportion"]
    * high_interacting_ct_props["receiver_weight"]
)

# Calculate weighted average for each sender type
weighted_props = (
    high_interacting_ct_props.groupby("neighbor_label")
    .agg({"weighted_proportion": "sum", "receiver_weight": "sum"})
    .reset_index()
)
weighted_props["final_proportion"] = (
    weighted_props["weighted_proportion"] / weighted_props["receiver_weight"]
)

print("Weighted sender proportions:")
for _, row in weighted_props.sort_values(
    "final_proportion", ascending=False
).iterrows():
...
filtered_proportions = high_interacting_ct_props_detailed[
    (
        high_interacting_ct_props_detailed["receiver_label"].isin(
            receiver_proportions.index
        )
    )
    & (
        high_interacting_ct_props_detailed["neighbor_label"].isin(
            sender_proportions.index
        )
    )
...
node_labels = [f"Sender_{ct}" for ct in sender_types] + [
    f"Receiver_{ct}" for ct in receiver_types
]
...
for i, k1 in enumerate(hub_results_dict.keys()):
    for j, k2 in enumerate(hub_results_dict.keys()):
        ari = adjusted_rand_score(
            hub_results_dict[k1].loc[adata.obs_names, "hub_cluster"].values,
            hub_results_dict[k2].loc[adata.obs_names, "hub_cluster"].values,
        )
...
if not fixed_q_key:
    ...
    for i, k1 in enumerate(q_k_set):
        for j, k2 in enumerate(q_k_set):
            hub_labels_1 = (
                hub_results_dict[(fixed_quantile, k1)]
                .loc[adata.obs_names, "hub_cluster"]
                .values
            )
            hub_labels_2 = (
                hub_results_dict[(fixed_quantile, k2)]
                .loc[adata.obs_names, "hub_cluster"]
                .values
            )
    ...
    plt.tight_layout()
    plt.savefig(
        f"figures/xenium_hub_grid_search_heatmaps_q{fixed_quantile}.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        f"figures/xenium_hub_grid_search_heatmaps_q{fixed_quantile}.svg",
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
...
for i, q1 in enumerate(q_set):
    for j, q2 in enumerate(q_set):
        hub_labels_1 = (
            hub_results_dict[(q1, fixed_n_clusters)]
            .loc[adata.obs_names, "hub_cluster"]
            .values
        )
        hub_labels_2 = (
            hub_results_dict[(q2, fixed_n_clusters)]
            .loc[adata.obs_names, "hub_cluster"]
            .values
        )
...
im0 = sns.heatmap(
    ...
    annot=True,
    fmt=".2f",
    xticklabels=np.round(q_set, 3),
    yticklabels=np.round(q_set, 3),
    ax=ax[0],
    cmap="Blues",
    vmin=0,
    vmax=1,
...
plt.tight_layout()
plt.savefig(
    f"figures/xenium_hub_grid_search_heatmaps_k{fixed_n_clusters}.png",
    dpi=300,
    bbox_inches="tight",
)
plt.savefig(
    f"figures/xenium_hub_grid_search_heatmaps_k{fixed_n_clusters}.svg",
    dpi=300,
    bbox_inches="tight",
)
plt.show()
