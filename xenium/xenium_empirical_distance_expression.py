...
def plot_profiles(
    ...
    fig.suptitle(
        "Xenium breast cancer: receiver expression vs surface-to-surface distance to nearest sender", y=1.0, fontsize=13
    )
    fig.tight_layout()
    for ext in ("png", "svg"):
        fig.savefig(
            os.path.join(OUTPUT_DIR, f"xenium_empirical_distance_expression{suffix}.{ext}"),
            dpi=300,
            bbox_inches="tight",
        )
    plt.close(fig)
