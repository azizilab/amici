    ...
    for seed in seeds:
        try:
            amici_pr = pd.read_csv(
                os.path.join(results_path, f"{snakemake.wildcards.dataset}_{seed}/amici_receiver_subtype_task_pr.csv")  # noqa: F821
            )
            ...
            if include_cgcom:
                cgcom_pr = pd.read_csv(
                    os.path.join(
                        results_path,
                        f"{snakemake.wildcards.dataset}_{seed}/cgcom_receiver_subtype_task_pr.csv",  # noqa: F821
                    )
                )
