...
class UnconstrainedAttentionAMICI(AMICI):
    """AMICI wrapper for loading cached unconstrained positional-attention models."""

    _module_cls = AMICIUnconstrainedAttentionModule


class UnimodalAttentionAMICI(AMICI):
    """AMICI wrapper that swaps in the unimodal Huber distance-bias module."""

    _module_cls = AMICIUnimodalAttentionModule


# %% Config
RUN_NAME = "unimodal_attention_sweep"
...
def train_or_load_run(adata, dataset_config, run, run_path, result_path, eval_indices):
    """Train one unimodal AMICI run, load its cached result, or record a failed run."""
...
def process_dataset_type(dataset_config, benchmark_dir, data_dir, model_dir, results_dir):
    """Train/evaluate unimodal AMICI across one dataset type."""
...
def plot_task_boxplots(results_df, figures_dir):
    ...
    fig.suptitle("Unimodal Huber-bias AMICI task performance across dataset seeds", fontsize=13)
    plt.tight_layout()
    for ext in ("png", "svg"):
        fig.savefig(
...
def plot_variant_comparison(unimodal_df, base_dir, figures_dir):
    """Save AUPRC boxplots comparing cached unconstrained and unimodal runs."""
    unconstrained_path = first_existing_path(
        os.path.join(
            base_dir,
            "results",
    ...
    if not os.path.exists(unconstrained_path):
        print(f"Missing cached unconstrained results: {unconstrained_path}")
        return

    unconstrained_df = pd.read_csv(unconstrained_path)
    unconstrained_df["variant"] = "Unconstrained"
    unimodal_df = unimodal_df.copy()
    unimodal_df["variant"] = "Unimodal"
    plot_df = pd.concat([unconstrained_df, unimodal_df], ignore_index=True)
    ...
    variants = ["Unconstrained", "Unimodal"]
    colors = {"Unconstrained": "#6B7280", "Unimodal": "#2E5BBA"}

    fig, axes = plt.subplots(1, 2, figsize=(13, 5), sharey=True)
    rng = np.random.default_rng(0)
    width = 0.32
    offsets = {"Unconstrained": -width / 2, "Unimodal": width / 2}
    for ax, dataset_type in zip(axes, dataset_types, strict=False):
        ...
        for variant in variants:
            ...
            data = [
                subset.loc[
                    (subset["variant"] == variant) & (subset["task"] == task),
                    "auprc",
                ]
                .dropna()
                .to_numpy()
            ...
            for x_pos, values in zip(positions, data, strict=False):
                ...
                ax.scatter(x_pos + jitter, values, color="black", s=14, alpha=0.55, zorder=3)

        handles = [plt.Line2D([0], [0], color=colors[variant], lw=8, alpha=0.5, label=variant) for variant in variants]
    ...
    fig.suptitle("Flexible AMICI variant task performance across matched dataset seeds", fontsize=13)
    plt.tight_layout()
    for ext in ("png", "svg"):
        fig.savefig(
            os.path.join(figures_dir, f"{RUN_NAME}_vs_unconstrained_task_auprc_boxplots.{ext}"),
            dpi=300,
            bbox_inches="tight",
        )
    plt.close(fig)
...
def interpolate_pr_curve(precision, recall, auprc, metadata, n_points=201):
    """Return a compact precision-recall curve on a fixed recall grid."""
    order = np.argsort(recall)
    recall_sorted = np.asarray(recall)[order]
    precision_sorted = np.asarray(precision)[order]
    grid = np.linspace(0.0, 1.0, n_points)
    interp_precision = np.interp(grid, recall_sorted, precision_sorted)
    return pd.DataFrame(
        {
            **metadata,
            "recall": grid,
...
def compute_pr_curves(model, adata, dataset_config, metadata):
    """Compute compact PR curves for all benchmark tasks."""
    gt_gene_scores_df, gt_neighbor_classes_df, gt_receiver_classes_df = build_task_ground_truth(adata, dataset_config)
    records = []
    ...
    records.append(interpolate_pr_curve(precision, recall, auprc, {**metadata, "task": "Gene"}))
    ...
    records.append(interpolate_pr_curve(precision, recall, auprc, {**metadata, "task": "Neighbor interaction"}))
    ...
    records.append(interpolate_pr_curve(precision, recall, auprc, {**metadata, "task": "Receiver subtype"}))
    return pd.concat(records, ignore_index=True)


def compute_cached_variant_pr_curves(
    variant_name,
    model_cls,
    variant_model_dir,
    output_dir,
    ...
    """Compute PR curves from cached best models without training."""
    os.makedirs(output_dir, exist_ok=True)
    safe_variant = variant_name.lower().replace(" ", "_")
    out_path = os.path.join(output_dir, f"{safe_variant}_pr_curves.csv")
    ...
    for dataset_config in dataset_configs:
        ...
        for dataset_seed in dataset_config["seeds"]:
            if dataset_type == "synthetic":
                ...
                adata, _ = generate_or_load_realistic_dataset(
                    benchmark_dir,
                    dataset_config,
                    data_dir,
                    dataset_seed,
            ...
            dataset_model_dir = os.path.join(variant_model_dir, dataset_type, f"dataset_seed_{dataset_seed}")
            best_model_path = os.path.join(dataset_model_dir, "best_model")
            best_params_path = os.path.join(dataset_model_dir, "best_model_params.json")
            if not (os.path.exists(os.path.join(best_model_path, "model.pt")) and os.path.exists(best_params_path)):
                print(f"Skipping {variant_name} {dataset_type} seed {dataset_seed}; cached best model is missing.")
                continue
            ...
            model = model_cls.load(best_model_path, adata=adata)
            metadata = {
            ...
            pr_records.append(compute_pr_curves(model, adata, dataset_config, metadata))

    if pr_records:
        pr_df = pd.concat(pr_records, ignore_index=True)
    else:
        pr_df = pd.DataFrame()
...
def plot_variant_pr_curves(pr_df, figures_dir):
    """Plot mean PR curves comparing unconstrained and unimodal variants."""
    if pr_df.empty:
        print("No PR curves available; skipping variant PR plot.")
        return
    ...
    fig, axes = plt.subplots(len(dataset_types), len(task_order), figsize=(14, 8), sharex=True, sharey=True)
    for row_idx, dataset_type in enumerate(dataset_types):
        for col_idx, task in enumerate(task_order):
            ax = axes[row_idx, col_idx]
            subset = pr_df[(pr_df["dataset_type"] == dataset_type) & (pr_df["task"] == task)]
            for variant, variant_df in subset.groupby("variant", sort=False):
                curve = variant_df.groupby("recall", as_index=False).agg(
                ...
                ax.plot(
                    ...
                    lw=2,
                    label=f"{variant} (AUPRC={curve['auprc'].mean():.3f})",
                )
            ax.set_title(f"{dataset_titles[dataset_type]}: {task}")
    ...
    fig.suptitle("Flexible AMICI variant precision-recall curves", fontsize=13)
    plt.tight_layout()
    for ext in ("png", "svg"):
        fig.savefig(
            os.path.join(figures_dir, f"{RUN_NAME}_vs_unconstrained_pr_curves.{ext}"),
            dpi=300,
            bbox_inches="tight",
        )
    plt.close(fig)
...
def plot_length_scale_distributions(length_scale_df, figures_dir):
    ...
    fig.suptitle("Unimodal Huber-bias AMICI length-scale distributions", fontsize=13)
    plt.tight_layout()
    for ext in ("png", "svg"):
        fig.savefig(
...
results_df.to_csv(os.path.join(results_dir, "unimodal_attention_task_scores.csv"), index=False)
plot_task_boxplots(results_df, figures_dir)
plot_variant_comparison(results_df, base_dir, figures_dir)

unconstrained_model_dir = first_existing_path(
    os.path.join(base_dir, "saved_models", "unconstrained_attention_sweep"),
    os.path.join(base_dir, "saved_models", "unconstrained_attention_analysis"),
)
unconstrained_results_dir = first_existing_path(
    os.path.join(base_dir, "results", "unconstrained_attention_sweep"),
    os.path.join(base_dir, "results", "unconstrained_attention_analysis"),
)
comparison_pr_curves = [
    compute_cached_variant_pr_curves(
        "Unconstrained",
        UnconstrainedAttentionAMICI,
        unconstrained_model_dir,
        unconstrained_results_dir,
        benchmark_dir,
        data_dir,
        (SYNTHETIC_CONFIG, REALISTIC_CONFIG),
    ),
    compute_cached_variant_pr_curves(
        "Unimodal",
        UnimodalAttentionAMICI,
        model_dir,
        results_dir,
        benchmark_dir,
        data_dir,
...
plot_variant_pr_curves(pd.concat(comparison_pr_curves, ignore_index=True), figures_dir)
...
length_scale_df.to_csv(os.path.join(results_dir, "unimodal_attention_length_scale_samples.csv"), index=False)
