...
EXTERNAL_CURRENT_AMICI_CACHES = {
    "synthetic": os.path.join(SENSITIVITY_DIR, "saved_models", "length_scale_dataset_bootstrap_ci"),
    "realistic_semisynthetic": os.path.join(
        SENSITIVITY_DIR,
        "saved_models",
        "realistic_length_scale_dataset_bootstrap_ci",
    ),
}


def benchmark_path(path):
    """Resolve benchmark config paths relative to benchmarks/."""
...
def set_all_seeds(seed):
    """Set random seeds for reproducible model and dataset generation."""
    random.seed(seed)
    np.random.seed(seed)
...
def build_run_configs(dataset_config):
    """Build AMICI sweep configs from a dataset config."""
...
def cache_matches_run(result, run):
    """Return whether an existing result matches the requested config."""
    keys = [
        "dataset_type",
        "dataset_seed",
        "run_idx",
...
def generate_or_load_realistic_dataset(dataset_config, data_dir, dataset_seed):
    ...
    adata_path = os.path.join(data_dir, "amici_variants", f"{dataset_name}_{dataset_seed}.h5ad")
...
def build_task_ground_truth(adata, dataset_config):
    """Compute ground-truth labels for benchmark tasks."""
    ...
    gt_neighbor_classes_df = get_interaction_gt_neighbor_classes(adata, dataset_config["gt_interactions"], labels_key)
...
def train_or_load_run(adata, dataset_config, run, run_path, result_path, eval_indices):
    """Train one standard AMICI run or load a cached result."""
...
def external_cache_dir(dataset_type, dataset_seed):
    """Return a matching standard AMICI cache directory when available."""
    cache_root = EXTERNAL_CURRENT_AMICI_CACHES.get(dataset_type)
    if cache_root is None:
        return None
    if dataset_type == "synthetic":
        cache_dir = os.path.join(cache_root, f"dataset_{dataset_seed}")
    else:
        cache_dir = os.path.join(cache_root, f"dataset_seed_{dataset_seed}")
    if os.path.exists(os.path.join(cache_dir, "best_model", "model.pt")):
        return cache_dir
    return None


def import_external_best_model(
    adata,
    dataset_config,
    dataset_seed,
    adata_path,
    ...
    """Copy and evaluate an existing standard AMICI best model."""
    dataset_type = dataset_config["dataset_type"]
    source_dir = external_cache_dir(dataset_type, dataset_seed)
    if source_dir is None:
        return None

    source_params_path = os.path.join(source_dir, "best_model_params.json")
    with open(source_params_path) as f:
        best_result = json.load(f)

    run_defaults = build_run_configs(dataset_config)[0]
    best_result = {
        **run_defaults,
        **best_result,
        "dataset_type": dataset_type,
        "dataset_seed": dataset_seed,
        "source_cache": source_dir,
        "selection_metric": best_result.get("selection_metric", "test_loss"),
    }
    if os.path.exists(best_model_path):
        shutil.rmtree(best_model_path)
    shutil.copytree(os.path.join(source_dir, "best_model"), best_model_path)
    best_result["model_path"] = best_model_path
    ...
    task_scores_df["best_test_loss"] = best_result.get("test_loss", np.nan)
    task_scores_df["best_train_seed"] = best_result.get("train_seed", np.nan)
    task_scores_df["best_end_val"] = best_result.get("end_val", np.nan)
    task_scores_df["best_value_l1"] = best_result.get("value_l1", np.nan)
    task_scores_df["n_successful_runs"] = best_result.get("n_successful_runs", np.nan)
...
def process_dataset_type(dataset_config, data_dir, model_dir, results_dir):
    """Train/evaluate standard AMICI across one dataset type."""
    dataset_type = dataset_config["dataset_type"]
    ...
    for dataset_seed in dataset_config["seeds"]:
        ...
        if dataset_type == "synthetic":
            ...
            adata, adata_path = generate_or_load_realistic_dataset(dataset_config, data_dir, dataset_seed)
        ...
        if os.path.exists(task_scores_path) and os.path.exists(best_params_path):
            dataset_results.append(pd.read_csv(task_scores_path))
            continue

        imported_scores = import_external_best_model(
            adata,
            dataset_config,
            dataset_seed,
            adata_path,
        ...
        if imported_scores is not None:
            print(f"Imported cached standard AMICI model for {dataset_type} seed {dataset_seed}")
            dataset_results.append(imported_scores)
            continue
        ...
        for run_idx, run_config in enumerate(run_configs):
            run = {**run_config, "dataset_type": dataset_type, "dataset_seed": dataset_seed, "run_idx": run_idx}
            ...
            task_scores_df = pd.DataFrame(
                {
                    ...
                    "error": ["No successful training runs"] * 3,
                }
            )
            task_scores_df.to_csv(task_scores_path, index=False)
    ...
    if run_records:
        pd.DataFrame(run_records).to_csv(os.path.join(results_dir, f"{dataset_type}_run_results.csv"), index=False)
    return pd.concat(dataset_results, ignore_index=True)


def main():
    """Run the standard AMICI variant sweep, using cached runs when available."""
    select_gpu()
    data_dir = os.path.join(SENSITIVITY_DIR, "data")
    model_dir = os.path.join(SENSITIVITY_DIR, "saved_models", "amici_variants", RUN_NAME)
    results_dir = os.path.join(SENSITIVITY_DIR, "results", "amici_variants", RUN_NAME)
    os.makedirs(model_dir, exist_ok=True)
    os.makedirs(results_dir, exist_ok=True)

    all_results = []
    for config in (SYNTHETIC_CONFIG, REALISTIC_CONFIG):
        all_results.append(process_dataset_type(config, data_dir, model_dir, results_dir))

    results_df = pd.concat(all_results, ignore_index=True)
    results_df.to_csv(os.path.join(results_dir, "current_amici_task_scores.csv"), index=False)
