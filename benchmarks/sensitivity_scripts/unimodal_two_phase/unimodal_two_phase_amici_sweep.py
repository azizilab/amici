...
# %% Config
RUN_NAME = "unimodal_two_phase_amici_sweep"
DATASET_SEED = 42
DATASET_NAME = "3ct_dataset_1int_unimodal_10_30"
GCS_DATASET_URI = ""
...
INTERACTION_PEAK = 20
INTERACTION_END = 30
INTERACTION_SIGMA = 4.0
INTERACTION_MIN_PROBABILITY = 0.05
...
GT_INTERACTIONS = {
    "interaction_unimodal": {
        ...
        # get_interaction_gt_neighbor_classes uses this band (not length_scale, which is kept above for the
        # length-scale-recovery comparison against AMICI's estimated peak) since the true interaction only
        # ever occurs for nearest-sender distances in [INTERACTION_START, INTERACTION_END].
...
def build_unimodal_interaction_df():
    """Build interaction metadata for one smooth unimodal sender-receiver pair."""
    return pd.DataFrame(
        [
            ...
            {"receptor_cell": "2", "receptor_subtype": "2_sub0", "interaction_type": "neutral"},
            {
                ...
                "radius_of_effect": INTERACTION_END,
...
def generate_unimodal_spatial_data(adata, num_cells=20000, rect_length=2000, rect_width=1000):
    """Generate 3-cell-type data with a probabilistic unimodal interaction.
    ...
    interaction_mask = (min_dist >= INTERACTION_START) & (min_dist <= INTERACTION_END)
    distances = min_dist[interaction_mask]
    probability = INTERACTION_MIN_PROBABILITY + (1 - INTERACTION_MIN_PROBABILITY) * np.exp(
        -((distances - INTERACTION_PEAK) ** 2) / (2 * INTERACTION_SIGMA**2)
    )
    sampled_interactions = np.random.random(len(distances)) < probability
    interaction_indices = receiver_indices[interaction_mask][sampled_interactions]
    spatial_data.loc[interaction_indices, "Subtype"] = "0_sub1"
...
def generate_unimodal_dataset(output_path):
    """Generate the unimodal synthetic dataset from PBMC source data."""
    ...
    interaction_df = build_unimodal_interaction_df()
    ...
    adata = generate_unimodal_spatial_data(adata_pp)
    adata = _create_train_test_split(adata)
    adata.obs_names_make_unique()
    adata.write_h5ad(output_path)


def ensure_unimodal_dataset():
    """Load the unimodal dataset, generating it if needed."""
    adata_path = os.path.join(DATA_DIR, f"{DATASET_NAME}.h5ad")
    if not os.path.exists(adata_path):
        set_all_seeds(DATASET_SEED)
        generate_unimodal_dataset(adata_path)
...
def plot_task_scores(task_scores_df):
    ...
    ax.set_ylim(0, 1)
    ax.set_ylabel("AUPRC")
    ax.set_title("Unimodal AMICI on Synthetic Dataset")
    ax.tick_params(axis="x", rotation=20)
    fig.tight_layout()
    fig.savefig(os.path.join(FIGURE_DIR, "unimodal_two_phase_task_auprc.png"), dpi=300)
    plt.close(fig)


def compute_length_scale_estimates(adata, model, best_result):
    """Compute best-head AMICI length scales for the unimodal interaction."""
...
def plot_length_scale_distributions(samples_df, estimates_df):
    ...
    ax.set_title("Unimodal synthetic AMICI preferred-distance recovery")
    ax.grid(axis="x", alpha=0.25)
    ax.legend(frameon=False, loc="lower right")
    fig.tight_layout()
    for ext in ("png", "svg"):
        fig.savefig(
            os.path.join(FIGURE_DIR, f"unimodal_two_phase_length_scale_distributions.{ext}"),
            dpi=300,
            bbox_inches="tight",
        )
    plt.close(fig)
...
# %% Main
def main():
    """Run the unimodal-interaction AMICI sweep."""
    select_gpu()
    adata, adata_path = ensure_unimodal_dataset()
    ...
    run_results_df.to_csv(os.path.join(RESULTS_DIR, "unimodal_two_phase_run_results.csv"), index=False)
    ...
    task_scores_df.to_csv(os.path.join(RESULTS_DIR, "unimodal_two_phase_task_scores.csv"), index=False)
    ...
    length_scale_estimates_df.to_csv(
        os.path.join(RESULTS_DIR, "unimodal_two_phase_length_scale_estimates.csv"), index=False
    )
    length_scale_samples_df.to_csv(
        os.path.join(RESULTS_DIR, "unimodal_two_phase_length_scale_samples.csv"), index=False
    )
