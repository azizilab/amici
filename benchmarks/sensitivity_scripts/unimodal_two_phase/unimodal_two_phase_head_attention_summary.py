...
def plot_attention_curves(curve_df):
    ...
    for ax, head_idx in zip(axes, heads, strict=False):
        ...
        ax.axvline(INTERACTION_START, color="black", linestyle="--", linewidth=1.3)
        ax.axvline(INTERACTION_PEAK, color="#2ca02c", linestyle="-", linewidth=1.6)
        ax.axvline(INTERACTION_END, color="black", linestyle=":", linewidth=1.6)
    ...
    fig.suptitle("Unimodal sender-to-receiver attention by head", y=1.02)
    fig.tight_layout()
    for ext in ("png", "svg"):
        fig.savefig(
            os.path.join(FIGURE_DIR, f"unimodal_two_phase_head_attention_summary.{ext}"), dpi=300, bbox_inches="tight"
        )
    plt.close(fig)
...
def plot_length_scale_by_head(length_df):
    ...
    ax.axhline(INTERACTION_PEAK, color="#2ca02c", linestyle="-", linewidth=1.8, label="GT peak distance 20")
    ax.set_xlabel("Attention head")
    ax.set_ylabel("Length scale at attention threshold 0.1")
    ax.set_title("Unimodal threshold-derived length scales by head")
    ax.legend(frameon=False)
    ax.grid(axis="y", alpha=0.25)
    fig.tight_layout()
    for ext in ("png", "svg"):
        fig.savefig(
            os.path.join(FIGURE_DIR, f"unimodal_two_phase_head_length_scale_summary.{ext}"),
            dpi=300,
            bbox_inches="tight",
        )
    plt.close(fig)
...
curve_df.to_csv(os.path.join(RESULTS_DIR, "unimodal_two_phase_head_attention_curves.csv"), index=False)
length_df.to_csv(os.path.join(RESULTS_DIR, "unimodal_two_phase_head_length_scale_summary.csv"), index=False)
