...
INTERACTION_HEADS = {
    "unimodal peak": {"head_idx": 3, "gt_length_scale": 20, "interaction_subtype": "0_sub1"},
}
...
def plot_selected_head_length_scales(length_df):
    """Plot selected-head length-scale distributions for the unimodal interaction."""
    order = ["unimodal peak"]
    label_map = {
        interaction: (
    ...
    ax.set_ylabel("")
    ax.set_title("Unimodal selected-head length-scale distribution")
    ax.grid(axis="x", alpha=0.25)
    fig.tight_layout()
    for ext in ("png", "svg"):
        fig.savefig(
            os.path.join(FIGURE_DIR, f"unimodal_two_phase_selected_head_length_scale_distributions.{ext}"),
            dpi=300,
            bbox_inches="tight",
        )
    plt.close(fig)
...
selected_length_df.to_csv(
    os.path.join(RESULTS_DIR, "unimodal_two_phase_selected_head_length_scale_samples.csv"),
    index=False,
)
