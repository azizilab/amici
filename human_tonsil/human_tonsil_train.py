...
COORD_KEY = "spatial"
DATA_DIR = "data"
PROJECT_NAME = f"human_tonsil_sweep_{DATA_DATE}_model_{date.today()}"
ENTITY_NAME = "stattention"
SAVED_MODELS_DIR = f"saved_models/human_tonsil_sweep_{DATA_DATE}_model_{date.today()}"

adata = ad.read_h5ad(f"{DATA_DIR}/human_tonsil_filtered_{DATA_DATE}.h5ad")
adata_train = ad.read_h5ad(f"{DATA_DIR}/human_tonsil_filtered_train_{DATA_DATE}.h5ad")
adata_test = ad.read_h5ad(f"{DATA_DIR}/human_tonsil_filtered_test_{DATA_DATE}.h5ad")
...
def train():
    ...
    try:
        ...
        model_params = {
            "n_heads": int(get_config_value("n_heads", 12)),
            "n_query_dim": 128,
            "n_head_size": 32,
            "n_nn_embed": 256,
            ...
            "value_l1_penalty_coef": float(get_config_value("value_l1_penalty_coef", 1.0e-4)),
        }
        exp_params = {
        ...
        model_path = os.path.join(
            SAVED_MODELS_DIR,
            f"human_tonsil_{seed}_sweep_{run.sweep_id}_{run.id}_params_{date.today()}",
        )
        if not os.path.exists(model_path):
            model.train(
                ...
                wandb_run_name=f"human_tonsil_{seed}_sweep_{run.sweep_id}_{run.id}",
                callbacks=[
