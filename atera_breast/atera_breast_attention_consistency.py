...
# attention frame for 169k cells x 16 heads x 50 neighbors never has to fit in memory.
CHUNK_SIZE = 20000
BATCH_SIZE = 128
...
def plot_consistency(runs_df, correlations):
    ...
    fig.tight_layout()
    for ext in ("png", "svg"):
        fig.savefig(os.path.join(FIGURE_DIR, f"atera_breast_attention_consistency.{ext}"), dpi=300)
    plt.close(fig)
    return runs_df
...
# %% Main
def main():
    """Compute and plot cross-run attention consistency for the atera breast sweep."""
    ...
    runs_df.to_csv(os.path.join(RESULTS_DIR, "atera_breast_attention_consistency_runs.csv"), index=False)
    pd.DataFrame(correlations, index=runs_df["run_id"], columns=runs_df["run_id"]).to_csv(
        os.path.join(RESULTS_DIR, "atera_breast_attention_consistency_correlations.csv")
    )
