...
Sensitivity analysis scripts (S20--S25) are standalone and train their own models internally.
...
- **Fig S20**: Number of attention heads sensitivity (AUPRC boxplots + PR curves)
...
- **Fig S21**: Number of neighbors sensitivity (AUPRC boxplots + PR curves)
...
- **Fig S22**: Value L1 penalty sensitivity (AUPRC boxplots + PR curves)
...
- **Fig S23**: Attention penalty sensitivity (AUPRC boxplots + PR curves)
...
- **Fig S25**: Length scale sensitivity (boxplot of d_scale vs alpha threshold)
...
- **Fig S3**: Full directed interaction network of all cortex cell types (`plot_interaction_directed_graph()`)
...
- **Fig S4c**: Explained variance by attention head (`plot_explained_variance_barplot()`)
...
- **Fig S5a**: Hub vs composition cluster spatial comparison
- **Fig S5b**: ARI/AMI comparison between hubs, composition clusters, and cell-type labels
- **Fig S12a**: Silhouette score vs number of clusters
- **Fig S12b**: Alluvial/Sankey diagrams for all 10 communication hubs
- **Fig S26**: Hub grid search heatmaps — fixed k, varying quantile threshold
- **Fig S27**: Hub grid search heatmaps — fixed quantile, varying k
...
- **Fig S13**: GSEA barplots comparing hub-unique vs shared vs composition-unique significant pathways
- **Fig S14**: GSEA butterfly charts — tumor cell types (MSigDB Hallmark)
- **Fig S15**: GSEA butterfly charts — immune cell types (Reactome)
- **Fig S16**: GSEA butterfly charts — stromal cell types (Reactome)
...
- **Fig S4a**: Interaction strength heatmaps for replicate 1 and replicate 2
- **Fig S4b**: Replicate scatter plot with Spearman correlation
...
- **Fig S24**: Null z-value distribution histogram + Q-Q plot
...
- **Fig S19**: Runtime benchmark (CPU wall-clock time + GPU seconds/epoch vs number of cells)
...
- **Fig S17**: Low-resolution comparison matrices (high-res, aggregated, and low-res-trained)
- **Fig S18**: Interaction matrix similarity vs shuffled null (cosine similarity and Pearson r histograms)
